...
function timeToAngle(time: `${string}:${string}`) {
...
export function ClockView({ items, categories, onItemClick }: ClockViewProps) {
  const [tooltip, setTooltip] = useState<TooltipState>({ visible: false, x: 0, y: 0, item: null });
  const hourMarkers = Array.from({ length: HOURS_IN_DAY }, function createHour(_, i) {
    return i;
  });
  ...
  function handleMouseEnter(event: React.MouseEvent, item: ScheduleItem) {
  ...
  return (
    <div className="flex flex-col items-center">
          ...
          {items.map(function renderScheduleArc(item) {
            ...
            return (
              <path
                ...
                onClick={function handleArcClick(e) {
                  const centerAngle = (startAngle + endAngle) / 2;
                  const midRadius = (INNER_RADIUS + OUTER_RADIUS) / 2;
                  const point = polarToCartesian(CENTER, CENTER, midRadius, centerAngle);
                  const svgRect = (
                    e.currentTarget.ownerSVGElement as SVGSVGElement
                  ).getBoundingClientRect();
                  onItemClick?.(item, {
                    x: svgRect.left + point.x,
                    y: svgRect.top + point.y,
                  });
                }}
      ...
      {tooltip.visible && tooltip.item && (
        <div
          className="pointer-events-none fixed z-[9999] -translate-x-1/2 -translate-y-full transform rounded-lg bg-gray-800 px-3 py-2 text-xs text-white shadow-lg"
          style={{ left: tooltip.x, top: tooltip.y - 8 }}
        >
          <div className="mb-1 font-semibold">{tooltip.item.title}</div>
          ...
          <div className="absolute top-full left-1/2 h-0 w-0 -translate-x-1/2 border-t-4 border-r-4 border-l-4 border-transparent border-t-gray-800" />
        </div>
      )}
